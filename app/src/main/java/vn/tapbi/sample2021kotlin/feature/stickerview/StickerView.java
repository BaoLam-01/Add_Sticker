...
public class StickerView extends FrameLayout {
    ...
    private final float limitScale;
    ...
    public StickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            limitScale = a.getFloat(R.styleable.StickerView_limitScale, 0);
    ...
    public void configDefaultIcons() {
        BitmapStickerIcon deleteIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.close_sticker_icon),
                BitmapStickerIcon.LEFT_TOP);
        deleteIcon.setIconEvent(new DeleteIconEvent());
        BitmapStickerIcon zoomIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.zoom_sticker_icon),
                BitmapStickerIcon.RIGHT_BOTOM);
        zoomIcon.setIconEvent(new ZoomIconEvent());
        BitmapStickerIcon flipIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.flip_sticker_icon),
    ...
    protected boolean onTouchDown(@NonNull MotionEvent event) {
        ...
        oldSize = calculateDistance(bitmapPoints[0], bitmapPoints[1], bitmapPoints[6], bitmapPoints[7]);
...
//                        constrainSticker2(handlingSticker);
    ...
    protected float limitScale(float scale, float scaleLimit) {
...
//    }
    ...
    private float limitScaleSize(Sticker sticker, float newDistance) {
        ...
        float screenWith = Utils.INSTANCE.getScreenWidth(getContext());

        if (widthSticker(sticker) > screenWith * 1.5) {
            scaleFactor = (float) ((screenWith * 1.5) / sticker.getWidth());
            return scaleFactor;

        } else if (widthSticker(sticker) < 200) {
            scaleFactor = (float) 100 / sticker.getWidth();
            return scaleFactor;
        } else {
            return scaleFactor;
        }
    ...
    private float widthSticker(Sticker sticker) {
        RectF bounds = sticker.getMappedBound();
        return bounds.width();
    }
    ...
    protected void addStickerImmediately(@NonNull Sticker sticker, @Sticker.Position int position) {
        ...
        heightScaleFactor = (float) getHeight() / sticker.getWidth();
...
//        .postScale(scaleFactor / 2, scaleFactor / 2, getWidth() / 2, getHeight() / 2);
    ...
    protected void setStickerPosition(@NonNull Sticker sticker, @Sticker.Position int position) {
        ...
        if ((position & Sticker.Position.TOP) > 0) {
            ...
            offsetY *= 3f / 4f;
        } else if ((position & Sticker.Position.BEARD) > 0) {
            offsetY = offsetY / 2f + 70f;
        } else {
            offsetY /= 2f;
        }
